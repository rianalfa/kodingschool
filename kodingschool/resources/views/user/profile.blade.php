            ...
            <x-card.base title="Badge">
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-2 gap-4">
                    @forelse ($badges as $badge)
                        <div class="py-4">
                            <img src="{{ asset('images/badges/'.$badge->rank.'.png') }}" alt="Rank {{ $badge->rank }}"
                                class="w-24 h-24 object-cover mx-auto">
                            <x-badge.black class="text-center w-full mt-2">{{ date('F Y', strtotime($badge->created_at)) }}</x-badge.black>
                            <p class="text-sm text-center font-bold mb-0">{{ $badge->point }}pts</p>
                        </div>
                    @empty
